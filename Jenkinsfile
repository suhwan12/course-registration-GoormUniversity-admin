        ...
        stage('Update K8s to New Frontend Deployment'){
          steps{
            container('gradle'){
                ...
                sh 'sed -i "s/image:.*/image: suhwan11\\/frontend-admin:${BUILD_NUMBER}/g" front-deployment-admin.yaml'
                sh 'git config --global user.name suhwan12'
                sh 'git config --global user.email xman0120@naver.com'
                sh 'git config --global --add safe.directory /home/jenkins/agent/workspace/Frontend-Pipeline-Admin'
